...
All server-side tests should test SvelteKit remote functions. If you find yourself testing something that is not a SvelteKit remote function, ask the user first for guidance.

Remote function tests should be named as: `*.test.ts` or `*.spec.ts`

Do not append `.svelte.` to the filename, as that tells vitest to use the client browser environment.
